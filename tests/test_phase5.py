...
class TestLangGraphAdapter:
    def _mock_graph(self, reply: str = "LangGraph reply") -> MagicMock:
        graph = MagicMock()
        graph.ainvoke = AsyncMock(return_value={"messages": [MagicMock(content=reply)]})
        return graph
...
class TestAutoGenAdapter:
    def _mock_agent(self, summary: str = "AutoGen reply") -> MagicMock:
        agent = MagicMock()
        agent.a_initiate_chat = AsyncMock(return_value=MagicMock(summary=summary))
        return agent
    ...
    async def test_execute_returns_summary(self):
        ...
        with patch(
            "nexus_a2a.adapters.autogen.AutoGenAdapter._build_proxy"
        ) as mock_proxy:
            proxy = MagicMock()
...
class TestGoogleADKAdapter:
    ...
    async def test_execute_returns_output(self):
        ...
        with (
            patch.object(
                adapter, "_build_runner", new_callable=AsyncMock
            ) as mock_runner,
...
class TestMetricsCollector:
    ...
    def test_record_agent_call_context_manager(self):
        ...
        with patch(
            "nexus_a2a.storage.metrics.time.monotonic", side_effect=monotonic_values
        ):
            m = MetricsCollector()
